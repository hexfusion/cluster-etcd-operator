...
func (c *DefragController) checkDefrag(ctx context.Context, recorder events.Recorder) error {
	...
	if controlPlaneTopology != configv1.HighlyAvailableTopologyMode {
		klog.V(4).Infof("DefragController is disabled for control plane topology: %s", controlPlaneTopology)
		return nil
	}
	...
	for _, status := range endpointStatus {
		...
		if isEndpointFragmented(member, status) {
			...
			recorder.Eventf("DefragmentSuccess", "etcd member has been defragmented: %s, dbSize: %d, dbInUse: %d, member ID: %d, leader ID: %d", member.Name, postDefragEndpointStatus.DbSize, postDefragEndpointStatus.DbSizeInUse, member.ID, postDefragEndpointStatus.Leader)
...
// isEndpointFragmented checks the status of all cluster members to ensure that no members have a fragmented store.
...
func isEndpointFragmented(member *etcdserverpb.Member, endpointStatus *clientv3.StatusResponse) bool {
